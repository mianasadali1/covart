...
- (void)tappedMenuPanel:(UITapGestureRecognizer*)sender
{
    ...
    switch (view.tag) {
        ...
        case 1:
            ...
            break;
        case 2:
            [self addNewText:@"Double tap to edit text"];
            break;
        case 3:
            if(self.selectedTextView){
                [self addColorPicker];
...
- (id)initWithTool:(CLTextTool*)tool
{
    ...
    if(self){
        ...
        self.text = [NSAttributedString dvs_attributedStringWithString:@"Double tap to edit text"
                                                                              tracking:0
...
- (void)setText:(NSAttributedString *)text
{
    //if(![text.string isEqualToString:_text.string]){
        _text = text;
        _label.attributedText = (_text.string.length>0) ? _text : [[NSAttributedString alloc] initWithString:@"Double Tap to add text"];
   // }
}
