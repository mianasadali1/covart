...
//                imgSelected.image = self.imgSelected.image?.imageRotatedByDegrees(degrees: CGFloat(filterSlider.value))
    ...
    func createEdittedImage(filterName: String, image: UIImage) -> UIImage {
        ...
        if editName == "Warmth" {
            let warmX = (filterSlider.value * 2000) + 6500
            filter?.setValue(CIVector(x: CGFloat(warmX), y: 0), forKey: "inputNeutral")
            filter?.setValue(CIVector(x: 6500, y: 0), forKey: "inputTargetNeutral")
        } else if editName == "Vignette" {
            filter?.setValue(filterSlider.value * 2, forKey: "inputIntensity")
            filter?.setValue(filterSlider.value * 30, forKey: "inputRadius")
        } else {
            filter?.setValue(NSNumber(value: filterSlider.value), forKey: (editList[selectedAdjustment])["keyName"] as? String ?? "")
        }
