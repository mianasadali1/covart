...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if(self.isSelectingImage == YES) {
        ...
        viewImage.hidden = NO;
        viewImageHeightConstraint.constant = 260;
...
- (IBAction)actionButtonNext:(UIButton *)sender {
    ...
    NSString * viewControllerID = @"FirstVC";
    UIStoryboard * storyboard = [UIStoryboard storyboardWithName:storyboardName bundle:nil];
    
    FirstVC *controller = [storyboard instantiateViewControllerWithIdentifier:viewControllerID];
    controller.selectedImage = selectedImage.image;
...
-(void)photoSelected:(NSNotification *)notif {
    ...
    if(self.isSelectingImage == YES) {
        [self dismissViewControllerAnimated:YES completion:^{
            [[NSNotificationCenter defaultCenter] postNotificationName:@"ImageChanged" object:photo];
        }];
    }
}
